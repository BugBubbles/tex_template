...
以上公式的和式非常复杂，需要用到有限维的膜电位增量过程的分布，而该增量过程来自于泊松脉冲序列，因此该增量过程中，不重叠的时间区间内对应的增量是独立的，任意有限维的分布等于单个维度分布的有限次乘积，均可使用公式\ref{eq:膜电位增量的分布}表示。因此，其任意有限维的分布函数为：
\begin{align}
  f_{\Delta V}(v_1,v_2,\cdots,v_n,t)=\left\{
  \begin{aligned}
     & \circbrac{\frac{\lambda\tau}{v^{\lambda \tau+1}}e^{-\lambda t}}^n & ,e^{-\frac{t}{\tau}} \le v_1,v_2,\cdots,v_n \le 1 \\
     & 0                                                    & ,\textrm{其他}
  \end{aligned}
  \right.\label{eq:膜电位增量的有限维密度函数}
\end{align}\par
% 取$V_\varepsilon=\frac{\tau}{\tilde{w}}\circbrac{V_T-\varepsilon-\frac{V_0}{\tau}e^{-\frac{t}{\tau}}}$，根据全概率公式，有：\par
% \begin{align}
%   \begin{aligned}
...
\begin{align}
  \begin{aligned}
     & P\curlbrac{\frac{V_0}{\tau}e^{-\frac{t}{\tau}}+V(t_1+\Delta t)-V(t_1)\le V_T-\varepsilon\Big|0<\Delta t\le t_2-t_1} \\
     &=\sum_{n=1}^{\infty}P\curlbrac{\sum_{i=1}^{n}\Delta V_i(\Delta t)\le \frac{\tau}{\tilde{w}}\circbrac{V_T-\varepsilon-\frac{V_0}{\tau}e^{-\frac{\Delta t}{\tau}}}\Big| N(t_2)-N(t_1)=n,0<\Delta t\le t_2-t_1}\\
     & \quad\cdot  P\curlbrac{ N(t_2)-N(t_1)=n,0<\Delta t\le t_2-t_1}                                                      \\
  \end{aligned}
\end{align}\par
取$\displaystyle V_\varepsilon= \frac{\tau}{\tilde{w}}\circbrac{V_T-\varepsilon-\frac{V_0}{\tau}e^{-\frac{\Delta t}{\tau}}}$由于输入脉冲的数量对单个脉冲引起的增量事件是独立的，因此上式的条件概率可以直接使用公式\ref{eq:膜电位增量的有限维密度函数}表示， 使用概率密度代替各个概率$P\curlbrac{\Delta V_n=\Delta v_n|N,\Delta t}$，得到以下积分形式：
\begin{align}
  \begin{aligned}
     & \cprob{\sum_{i=1}^{n}\Delta V_i(\Delta t)\le V_\varepsilon}{N(t_2)-N(t_1)=n,0<\Delta t\le t_2-t_1}                                                                                                                    \\
     & =\int_{0}^{\pinfty}\int_{0}^{\pinfty}\cdots\int_{0}^{\pinfty}P\curlbrac{\Delta V_1\le V_\varepsilon-\sum_{i=2}^{n}\Delta v_i\Big|\Delta V_2=\Delta v_2,\Delta V_3=\Delta v_3,\cdots,\Delta V_n=\Delta v_n,N,\Delta t} \\
  ...
  \end{aligned}
\end{align}\par
忽略小量$\varepsilon$，进一步可化简得到：
\begin{align*}
  \begin{aligned}
     & \cprob{\sum_{i=1}^{n}\Delta V_i(\Delta t)\le V_\varepsilon}{N(t_2)-N(t_1)=n,0<\Delta t\le t_2-t_1}                                                                                                                    \\
     & =\int_{e^{-t/\tau}}^{1}\mathrm{d}\Delta v_n\int_{e^{-t/\tau}}^{1}\mathrm{d}\Delta v_{n-1}\cdots\int_{e^{-t/\tau}}^{1}\mathrm{d}\Delta v_{2}\int_{e^{-t/\tau}}^{V_\varepsilon-\sum_{i=2}^{n}\Delta v_i}\circbrac{\frac{\lambda\tau}{v^{\lambda \tau+1}}e^{-\lambda t}}^n \mathrm{d}\Delta v_1\\
     &=(\lambda\tau)^n(n\lambda t+n-1)\circbrac{1-e^{-\frac{t}{\tau}}}^ne^{n-1}+\frac{(n\lambda t)!}{(n\lambda t+n)!}\sum_{k=0}^{n-1}C_{n-1}^k\rectbrac{V_\varepsilon-k-(n-k-1)e^{-\frac{t}{\tau}}}^{-n\lambda t}\\
     &=(n\lambda t+n-1)\circbrac{1-e^{-\frac{t}{\tau}}}^ne^{-n\lambda t-n+1}+\frac{(n\lambda t)!}{(n\lambda t+n)!}\sum_{k=0}^{n-1}C_{n-1}^k\rectbrac{\frac{\tau}{\tilde{w}}V_T-\frac{V_0}{\tilde{w}}e^{-\frac{t}{\tau}}-k-(n-k-1)e^{-\frac{t}{\tau}}}^{-n\lambda t}\\
     &(t>0,n\in\mathbb{N}^*)
  \end{aligned}
\end{align*}\par
对此项结果再求关于输入脉冲数量的期望，即为概率分布结果。此项结果复杂，考虑进行近似化简，$\lambda t$一般值为整数且接近于10，因此第一项中$1-e^{-\frac{t}{\tau}}<1$，$e^{-n\lambda t-n+1}<<1$，即使乘以$n$，其仍然是远远小于第二项，可以将第一项忽略。在和式的第二项中，首先括号内的$V_0=0$。由于组合数$C_{n-1}^k$与$n$有关，$n$值越大，该组合数在$k=1,n-1$两侧的位置值相对于中间位置$k=(n-1)/2$或者$k=n/2$处差距越大，并且组合数所乘值随$k$值增大而减小。近似得到的结果如下：
\begin{align*}
  \begin{aligned}
     & \cprob{\sum_{i=1}^{n}\Delta V_i(\Delta t)\le V_\varepsilon}{N(t_2)-N(t_1)=n,0<\Delta t\le t_2-t_1}                                                                                                                    \\
     &=\frac{(n\lambda t)!}{(n\lambda t+n)!}\sum_{k=0}^{n-1}C_{n-1}^k\rectbrac{\frac{\tau}{\tilde{w}}V_T-\frac{V_0}{\tilde{w}}e^{-\frac{t}{\tau}}-k-(n-k-1)e^{-\frac{t}{\tau}}}^{-n\lambda t}
  \end{aligned}
\end{align*}\par
重要知识，教材上的第6.23题模型与我所推的模型高度相似！
\subsection{膜电位量}
脉冲神经元的膜电位决定了其输出脉冲序列的一切分布特性，因此首先分析的是膜电位量，设随机过程$V(t)$为脉冲神经元在时刻$t$的膜电位随机量。根据公式\ref{eq:LIF膜电位公式}可知，该随机过程是由输入随机序列$X(t)$与指数衰减过程的复合随机过程，输入随机序列服从泊松分布，因此可将膜电位量写成如下的复合随机过程形式：
\begin{align}
...
\end{align}\par
可见放电过程是非线性过程，其概率分布的直接表达式很难求解，这里结合公式\ref{eq:膜电位量的期望}，间接地求解其放电脉冲序列时间间隔，即有：
\begin{align}
  \expec{V(t)}=V_T\implies T=-\tau\ln\circbrac{1-\frac{V_T}{\lambda\tau}}=-\tau\ln\circbrac{1-\frac{V_T}{\lambda\tilde{w}}}
\end{align}\par
式中的$\lambda$对于输出脉冲序列而言是未知量，因此将$\lambda\tilde{w}$合并为新的权重，记为$\omega=\lambda\tilde{w}$，上式可以化简为：
\begin{align}
  T=-\tau\ln\circbrac{1-\frac{V_T}{\omega}}
  \label{eq:输出脉冲的到达时间间隔期望}
\end{align}\par
因此，在使用脉冲神经元时，可以结合公式\ref{eq:膜电位量的期望}和公式\ref{eq:输出脉冲的到达时间间隔期望}实现对权重的更新，下面将使用一个简单的二分类作为例子。
\subsection{分类实验}
如同感知神经元一样，使用脉冲神经元可以构建起脉冲神经网络，实现对输入信号的处理。这里，使用两个输出脉冲神经元，搭建一个二分类脉冲神经网络，实现对输入脉冲序列的分类。
\subsubsection{实验准备}
输入的脉冲序列按以下方式生成，准备一个只有两个实数的集合$A={1,2}$，使用噪声的方式在两个元素周边增加一系列噪声点，并向集合中插入，得到训练集$A={-0.9,1,1.2,1.8,2,2.3}$。\par
