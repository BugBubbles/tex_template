...
\begin{align}
  \begin{aligned}
    ...
    % \\
    %                       & =\sum_{i=1}^{\infty}P\curlbrac{\Delta V_i(t)\le \frac{\tau}{\tilde{w}}\circbrac{v-V_0e^{-\frac{t}{\tau}}}}                                                                                                        \\
    %                       & =\sum_{i=1}^{\infty}\curlbrac{1-\sum_{k=0}^{i-1}e^{-\lambda t}\frac{(\lambda t)^k}{k!}\cdot\curlbrac{1-\sum_{k=0}^{i-1}e^{-\lambda t(\frac{v\tau}{\tilde{w}})}\frac{[\lambda t(\frac{v\tau}{\tilde{w}})]^k}{k!}}}
  \end{aligned}
  \label{eq:膜电位的条件分布概率}
\end{align}\par
% 进一步算出膜电位的概率分布密度函数为：
...
%   f_{V(t)}(v)=\left\{
%   \begin{aligned}
%      & \frac{\lambda\tau}{v}\sum_{i=1}^{\infty}\sum_{k=0}^{i-1}e^{-\lambda t}\frac{(\lambda t)^k}{k!}e^{-\lambda t(\frac{v\tau}{\tilde{w}})}\frac{[\lambda t(\frac{v\tau}{\tilde{w}})]^{i-1}}{(i-1)!} & ,v\ge 0 \\
%      & 0                                                                                                                                                                                              & ,v<0
%   \end{aligned}\right.
%   \label{eq:膜电位的概率分布密度函数}
% \end{align}\par
...
\begin{align}
  F_{V(t)}(v)=    \sum_{n=1}^{\infty}P\curlbrac{\sum_{i=1}^{n}\Delta V_i(t)\le \frac{\tau}{\tilde{w}}\circbrac{v-\frac{V_0}{\tau}e^{-\frac{t}{\tau}}}\Big| N(t_2)-N(t_1)=n}P\curlbrac{ N(t_2)-N(t_1)=n}
  \label{eq:膜电位分布概率}
\end{align}\par
% 目标渐渐清楚了，需要计算出膜电位期望和方差，进一步算出输出脉冲列的期望和方差，以此可以完成分类任务，也可以计算膜电位增量、输出脉冲序列的到达时间间隔，以到达时间间隔的估计求取分类结果。推得出一个是一个，推得出全部本文可以考虑写小论文了。
\newpage
